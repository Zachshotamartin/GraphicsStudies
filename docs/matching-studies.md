...
**Bounds:** all images 4–256 pixels per side; requested levels 1–4, automatically limited so the coarsest images retain at least a few pixels; candidates 8–256; coherence 0–8. The browser preview currently uses up to 256 pixels per side and 128 candidates; lower resolutions remain useful on slower devices.
...
### Registered engraving exemplar

The Image Analogies demonstration uses its own architectural photographs rather than reusing another study’s subject or finished result. The example pair is a façade photograph A and a pixel-aligned engraving treatment A′ prepared by `src/algorithms/engraving-example.js`:

```js
import { engraveExample } from './src/algorithms/engraving-example.js';
const filtered = engraveExample({ source: A });
const frames = analogize({ source: A, filtered, target: B,
  levels: 3, coherence: 0.1, candidates: 128, seed: 42 });
```

The exemplar preparation is a deterministic image filter: lightly smoothed luminance drives Sobel contours, approximate nonmaximum suppression thins the contours, and progressively darker regions receive diagonal, crossed, and horizontal hatching. Warm off-white paper and dark ink make this demonstration visually distinct from the separate painterly-rendering study. The filter does not resize, shift, or warp A; A′ retains A’s dimensions and alpha, preserving the registration required for paired examples. Hatch spacing defaults to approximately one-sixtieth of the source width, with a three-pixel minimum. Edge strength defaults to 0.9 and hatch strength to 0.6, keeping the fine architectural contours legible. Both strengths, paper RGB, and ink RGB are configurable. The paired exemplar is stored as lossless PNG so its pixel values are preserved when used as A′.

**The engraving filter prepares only A′. It is not applied to B to obtain the displayed result.** B′ is generated by the independent multiscale Image Analogies algorithm, choosing pixels from the supplied A′ according to A/B appearance and synthesized-neighborhood correspondences. Changing the example pair changes the learned result. Unlike direct engraving of B, synthesis can produce broken hatching or misplaced details when there is no close exemplar match; these are genuine approximation limits, not concealed by a fallback filter. This is not a claim that the exemplar-preparation filter reproduces a separate published engraving algorithm.

Run `node --test tests/engraving-example.test.js` for registration, alpha preservation, immutable deterministic output, tone-sensitive hatching, contour alignment, invalid-input handling, and integration with actual analogy correspondences.
...
The image renderer applies the **forward** MLS map to a regular triangular grid, adding source pin coordinates to grid axes so pins fall on vertices. It rasterizes those forward-deformed triangles with barycentric texture coordinates and premultiplied-alpha bilinear pixel sampling. This avoids pretending that swapping source/destination pins gives an exact inverse deformation. With `boundary: 'free'` (the algorithm API default), uncovered result pixels are transparent, and geometry outside the original-sized canvas is cropped. With `boundary: 'fixed'`, every outer mesh vertex stays at its original image coordinate; interior mesh vertices and interior control pins still use the genuine MLS mapping. This keeps the rectangular image perimeter in place instead of covering empty corners with a fabricated paper layer. The interface defaults to a fixed image border and offers free movement as an explicit option. A moved pin whose original position lies on the fixed border is rejected with an explanation; move an interior pin or choose the free border instead. `mapPoint` remains the unconstrained continuous MLS evaluator because it has no image dimensions; the perimeter constraint belongs to the image mesh renderer.

**Bounds:** source 2–512 pixels per side; at most 32 handles; grid step 1–16; weight exponent 0.1–4. A smaller grid step samples the warp more accurately at greater cost. The result includes `vertices`, `triangles`, `mode`, and the chosen `boundary` alongside RGBA image data and `progress: 1`.

**Scope and limitations:** only point handles are implemented, not the paper's line-segment controls. The grid approximates the continuous map between vertices. Strong warps can fold over and overlap; later triangles draw over earlier ones, and no collision-free or invertible-map guarantee is made. Raster boundary antialiasing is not multisampled. The free-border view should show transparent/checkered uncovered regions honestly. Fixed borders preserve the frame but do not prevent interior fold-over under extreme pin movements; moderate initial movements remain appropriate.

## Validation

Run `node --test tests/matching-studies.test.js`.

The tests check known-pixel preservation, donor exclusion, erased-content independence, a quantitative repeated-pattern completion improvement, reproducibility, rejection of impossible inputs, paired-exemplar and target dependence, exact correspondence to A′, actual multiresolution progression, self-paired identity reconstruction, exact pin interpolation, translation, affine shear, similarity scaling, rigid rotation, degenerate handles, forward image rasterization, fixed-perimeter coverage with exact interior pin colors, and contradictory boundary-pin rejection. They verify mathematical behavior rather than snapshots of one implementation.
